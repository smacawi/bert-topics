...
ct_norm_cluster = pd.crosstab(pred, cluster, rownames=['Prediction'], colnames=['Cluster'], margins=False, normalize = 'columns').apply(lambda r: round(r, 2))
ct_norm_pred = pd.crosstab(pred, cluster, rownames=['Prediction'], colnames=['Cluster'], margins=False, normalize = 'index').apply(lambda r: round(r, 2))
